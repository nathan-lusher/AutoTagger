...
# model.save_weights(os.path.join(OUTPUT_DIR, "video_tagger_model_v3.weights.h5"))
...
from tensorflow.keras.layers import Input, TimeDistributed, GlobalMaxPooling1D, Dense
...
class VideoDataset(tf.keras.utils.Sequence):
    ...
    def __getitem__(self, idx):
        ...
        video_batch = []
        for frame_group in batch_frames:
            frames = [np.array(Image.open(fp).resize(IMAGE_SIZE)) / 255.0 for fp in frame_group]
            video_batch.append(frames)
...
def build_model(input_shape=(FRAMES_PER_VIDEO, 512, 512, 3), num_tags=100):
    ...
    x = TimeDistributed(base_cnn)(frame_input)
    x = GlobalMaxPooling1D()(x)
    x = Dense(num_tags, activation='sigmoid', dtype='float32')(x)  # Cast to float32 for stability
